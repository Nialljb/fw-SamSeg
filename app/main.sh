#! /bin/bash
#
# Run script for flywheel/mri-samseg Gear.
#
...
output_file=$OUTPUT_DIR/"$base_filename"'_samseg'
echo "output_file is $output_file"

# Check if user wanted parcelation output
if [[ $config_parc == 'true' ]]; then
  parc='--pallidum-separate'
fi

# Run samseg with options
if [[ -e $input_file ]]; then
  echo "Running samseg..."
  run_samseg --input $input_file --output $OUTPUT_DIR $parc
